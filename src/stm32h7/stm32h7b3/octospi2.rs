...
/// Access functions for the OCTOSPI1_CONTROL_REGISTER peripheral instance
pub mod OCTOSPI1_CONTROL_REGISTER {
    ...
    /// Reset values for each field in OCTOSPI1_CONTROL_REGISTER
    ...
    #[no_mangle]
    static mut OCTOSPI1_CONTROL_REGISTER_TAKEN: bool = false;

    /// Safe access to OCTOSPI1_CONTROL_REGISTER
    ///
    ...
    pub fn take() -> Option<Instance> {
        external_cortex_m::interrupt::free(|_| unsafe {
            if OCTOSPI1_CONTROL_REGISTER_TAKEN {
                None
            } else {
                OCTOSPI1_CONTROL_REGISTER_TAKEN = true;
                Some(INSTANCE)
            }
        })
    }

    /// Release exclusive access to OCTOSPI1_CONTROL_REGISTER
    ///
    ...
    pub fn release(inst: Instance) {
        external_cortex_m::interrupt::free(|_| unsafe {
            if OCTOSPI1_CONTROL_REGISTER_TAKEN && inst.addr == INSTANCE.addr {
                OCTOSPI1_CONTROL_REGISTER_TAKEN = false;
            } else {
    ...
    /// Unsafely steal OCTOSPI1_CONTROL_REGISTER
    ///
...
/// Raw pointer to OCTOSPI1_CONTROL_REGISTER
///
...
pub const OCTOSPI1_CONTROL_REGISTER: *const RegisterBlock = 0x52005000 as *const _;
...
/// This constant is provided for ease of use in unsafe code: you can
/// simply call for example `write_reg!(gpio, GPIOA, ODR, 1);`.
pub const OCTOSPI2: *const RegisterBlock = 0x5200a000 as *const _;
