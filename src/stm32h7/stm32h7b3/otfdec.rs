...
pub mod ICR {
    pub use super::ISR::KEIF;
    pub use super::ISR::SEIF;
    pub use super::ISR::XONEIF;
}
...
pub const OTFDEC1: *const RegisterBlock = 0x5200b800 as *const _;

/// Access functions for the OTFDEC2 peripheral instance
pub mod OTFDEC2 {
    use super::ResetValues;
    ...
    /// Reset values for each field in OTFDEC2
    pub const reset: ResetValues = ResetValues {
        CR: 0x00000000,
        R1CFGR: 0x00000000,
        R2CFGR: 0x00000000,
    ...
    #[allow(private_no_mangle_statics)]
    #[no_mangle]
    static mut OTFDEC2_TAKEN: bool = false;

    /// Safe access to OTFDEC2
    ///
    /// This function returns `Some(Instance)` if this instance is not
    /// currently taken, and `None` if it is. This ensures that if you
    /// do get `Some(Instance)`, you are ensured unique access to
    ...
    /// Release exclusive access to OTFDEC2
    ///
    /// This function allows you to return an `Instance` so that it
    /// is available to `take()` again. This function will panic if
    /// you return a different `Instance` or if this instance is not
    ...
    pub fn release(inst: Instance) {
        external_cortex_m::interrupt::free(|_| unsafe {
            if OTFDEC2_TAKEN && inst.addr == INSTANCE.addr {
                OTFDEC2_TAKEN = false;
            } else {
                panic!("Released a peripheral which was not taken");
            }
...
/// This constant is provided for ease of use in unsafe code: you can
/// simply call for example `write_reg!(gpio, GPIOA, ODR, 1);`.
pub const OTFDEC2: *const RegisterBlock = 0x5200bc00 as *const _;
