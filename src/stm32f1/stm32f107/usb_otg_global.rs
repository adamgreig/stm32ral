...
pub mod USB_OTG_GLOBAL {
    ...
    /// Reset values for each field in USB_OTG_GLOBAL
    ...
    #[no_mangle]
    static mut USB_OTG_GLOBAL_TAKEN: bool = false;

    /// Safe access to USB_OTG_GLOBAL
    ///
    ...
    pub fn take() -> Option<Instance> {
        external_cortex_m::interrupt::free(|_| unsafe {
            if USB_OTG_GLOBAL_TAKEN {
                None
            } else {
                USB_OTG_GLOBAL_TAKEN = true;
                Some(INSTANCE)
            }
        })
    }

    /// Release exclusive access to USB_OTG_GLOBAL
    ///
    ...
    pub fn release(inst: Instance) {
        external_cortex_m::interrupt::free(|_| unsafe {
            if USB_OTG_GLOBAL_TAKEN && inst.addr == INSTANCE.addr {
                USB_OTG_GLOBAL_TAKEN = false;
            } else {
...
pub const USB_OTG_GLOBAL: *const RegisterBlock = 0x50000000 as *const _;
