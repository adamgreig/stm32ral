...
/// Access functions for the USB_OTG_HOST peripheral instance
pub mod USB_OTG_HOST {
    ...
    /// Reset values for each field in USB_OTG_HOST
    ...
    #[no_mangle]
    static mut USB_OTG_HOST_TAKEN: bool = false;

    /// Safe access to USB_OTG_HOST
    ///
    ...
    pub fn take() -> Option<Instance> {
        external_cortex_m::interrupt::free(|_| unsafe {
            if USB_OTG_HOST_TAKEN {
                None
            } else {
                USB_OTG_HOST_TAKEN = true;
                Some(INSTANCE)
            }
        })
    }

    /// Release exclusive access to USB_OTG_HOST
    ///
    ...
    pub fn release(inst: Instance) {
        external_cortex_m::interrupt::free(|_| unsafe {
            if USB_OTG_HOST_TAKEN && inst.addr == INSTANCE.addr {
                USB_OTG_HOST_TAKEN = false;
            } else {
...
pub const USB_OTG_HOST: *const RegisterBlock = 0x50000400 as *const _;
